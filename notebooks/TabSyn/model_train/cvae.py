...
class VAE(nn.Module):
    ...
    def forward(self, x_num, x_cat):
        ...
        h = self.decoder(torch.cat((z, x_num.unsqueeze(-1).expand(-1, -1, 4)), dim=1))
...
class Reconstructor(nn.Module):
    def __init__(self, d_numerical, categories, d_token):
        ...
        self.lin = nn.Sequential(
            nn.Flatten(),
            nn.Linear(d_token *(d_numerical + self.d_categories+ d_numerical), d_token *(self.d_categories)),
            nn.ReLU(),
            nn.Unflatten(1, ((self.d_categories), d_token))
        )
        for d  in categories:
    ...
    def forward(self, h):
        ...
        h_num = h[:, -self.d_numerical:,:].sum(-1)
        h_cat = self.lin(h)
        # h_num = h[:, -self.d_numerical:,:].sum(-1)
        # h_cat  = h[:, :self.d_categories, :]
...
class Decoder_model(nn.Module):
    def __init__(self, d_numerical, categories, d_token, x_num, bias = True):
        super(Decoder_model, self).__init__()
        self.d_numerical = d_numerical
        self.d_categories = len(categories)
        self.d_token = d_token
        self.x_num = x_num
        self.VAE_Decoder = LM(d_token, self.d_numerical + self.d_categories + self.d_numerical)
    ...
    def forward(self, z):
        h = self.VAE_Decoder((torch.cat((z, self.x_num.unsqueeze(-1).expand(-1, -1, 4)), dim=1)))
