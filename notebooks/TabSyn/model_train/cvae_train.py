...
class VAE_main():
    def __init__(self, name, LR, WD, D_TOKEN):
    ...
    def tabsyn_vae_main(self, dataname = 'adult', ckpt_dir='model', max_beta=1e-2, min_beta=1e-5, lambd=0.7, device = 'cpu', num_epochs = 10):
        ...
        model = Model_VAE(d_numerical, categories, self.D_TOKEN, bias = True)
        model = model.to(device)

        pre_encoder = Encoder_model(d_numerical, categories, self.D_TOKEN).to(device)
        pre_decoder = Decoder_model(d_numerical, categories, self.D_TOKEN, x_num = X_num).to(device)
