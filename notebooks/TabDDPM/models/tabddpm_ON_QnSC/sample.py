...
@torch.no_grad()
def split_num_cat_target(syn_data, info, num_inverse):
    ...
    syn_num = np.array(num_inverse(syn_num)).astype(np.float32)
...
def sample(
    ...
    syn_num, syn_cat, syn_target = split_num_cat_target(syn_data, info, num_inverse) 
