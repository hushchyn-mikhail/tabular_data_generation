...
def _evaluate_binary_classification(train, test, info):
    ...
    for model_spec in classifiers:
        ...
        avg_roc_auc = results.loc[:, ['roc_auc']].mean().values[0]
        std_roc_auc = results.loc[:, ['roc_auc']].std().values[0]
...
def calculate_mle():
    ...
    if task_type == 'regression':
        ...
        best_f1_scores, best_weighted_scores, best_auroc_scores, best_acc_scores, best_avg_scores, avg_roc_auc, std_roc_auc = evaluator(train, test, info)
